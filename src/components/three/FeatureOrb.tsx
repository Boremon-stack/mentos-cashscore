  ...
  return (
    <group ref={ref}>
      <mesh>
        ...
        <meshStandardMaterial color="#bad797" emissive="#24301b" emissiveIntensity={0.6} roughness={0.3} metalness={0.5} />
      </mesh>
    </group>
  );
}
    ...
    <group>
      <group ref={outer}>
        <mesh>
          ...
          <meshStandardMaterial color="#b494a0" emissive="#3d1420" emissiveIntensity={0.7} roughness={0.25} metalness={0.6} />
        </mesh>
      </group>
      <group ref={inner}>
        <mesh position={[0, 0, 0.05]}>
          <torusGeometry args={[0.55, 0.09, 20, 48]} />
          <meshStandardMaterial color="#bad797" emissive="#24301b" emissiveIntensity={0.6} roughness={0.25} metalness={0.6} />
        </mesh>
      </group>
    </group>
  );
      ...
      {coins.map((i) => (
        <mesh key={i} position={[0, i * 0.32 - 0.5, 0]}>
          ...
          <meshStandardMaterial
            color={i % 2 === 0 ? "#bad797" : "#b494a0"}
            emissive={i % 2 === 0 ? "#24301b" : "#3d1420"}
    ...
    <div className="h-full w-full">
      <Canvas dpr={[1, 1.5]} camera={{ position: [0, 0, 3.4], fov: 40 }} gl={{ alpha: true }}>
        <Suspense fallback={null}>
          ...
          <pointLight position={[3, 3, 3]} intensity={1.3} color="#bad797" />
          <pointLight position={[-3, -2, 2]} intensity={1} color="#b494a0" />
